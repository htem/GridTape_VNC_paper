...
- **3.4** Return to step 3.1 to rerun alignment, but add `-t your_points_in_image.txt -tp your_points_in_template.txt` as arguments to your `run_elastix` command. Then, elastix will pull your corresponding points toward each other. Note that you can also add corresponding points to affine alignment commands (Step 2.1) via the same -t and -tp arguments.
